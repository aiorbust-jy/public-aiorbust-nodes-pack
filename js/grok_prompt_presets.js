/**
 * Aiorbust Grok Prompt Generator — Preset UI.
 *
...
async function loadPresetsFromServer() {
    try {
        const resp = await fetch("/grok_prompt/load_presets");
...
function buildPresetUI(node) {
    ...
    node.addWidget("button", "💾  Save current settings to preset slot", null, async () => {
        ...
        try {
            const resp = await fetch("/grok_prompt/save_preset", {
                method: "POST",
...
app.registerExtension({
    name: "aiorbust.GrokPromptPresets",

    async beforeRegisterNodeDef(nodeType, nodeData) {
        if (nodeData.name !== "GrokPromptNode") return;
