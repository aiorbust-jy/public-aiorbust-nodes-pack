# -*- coding: utf-8 -*-
"""
ComfyUI node - Aiorbust Save Video (no metadata).
...
class AiorbustSaveVideoNoMetadata:
        ...
        return {
            "required": {
                "filename_prefix": ("STRING", {
                    "default": "aiorbust", "multiline": False,
    ...
    def save(self, filename_prefix, method, video=None, video_path=None,
        ...
        prefix = (filename_prefix or "aiorbust").strip().replace("\\", "/")
        subdir, _, stem = prefix.rpartition("/")
        stem = "".join(c for c in stem if c.isalnum() or c in "-_") or "aiorbust"
        ...
        logging.info("Aiorbust Save Video (no metadata): %s", dest)

        return (dest, dest)


NODE_CLASS_MAPPINGS = {"AiorbustSaveVideoNoMetadata": AiorbustSaveVideoNoMetadata}
NODE_DISPLAY_NAME_MAPPINGS = {
    "AiorbustSaveVideoNoMetadata": "Aiorbust Save Video (no metadata)"}
