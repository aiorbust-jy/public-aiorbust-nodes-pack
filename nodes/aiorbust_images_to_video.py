# -*- coding: utf-8 -*-
"""
ComfyUI node - Aiorbust Images to Video (AB_VIDEO).

Turns an IMAGE batch into an .mp4 on disk and returns its path as AB_VIDEO, the
type `Aiorbust Image & Video Edit AIO` expects on `video_reference`.
...
class AiorbustImagesToVideo:
    ...
    def encode(self, images, fps, quality, filename_prefix,
        ...
        logging.info("Aiorbust Images to Video: %s", path)
        return (path, path)
