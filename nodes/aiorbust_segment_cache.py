# -*- coding: utf-8 -*-
"""
ComfyUI node - Aiorbust Segment Cache.
...
def clear_session(session: str) -> tuple:
    ...
    path = AiorbustSegmentCache._dir(session)
...
try:
    ...
    if not getattr(PromptServer.instance, "_aiorbust_segcache_routes", False):
        PromptServer.instance._aiorbust_segcache_routes = True

        @PromptServer.instance.routes.post("/aiorbust/segment_cache/clear")
        ...
        @PromptServer.instance.routes.get("/aiorbust/segment_cache/list")
    ...
    logging.warning("Aiorbust Segment Cache: routes not registered (server unavailable)")


NODE_CLASS_MAPPINGS = {"AiorbustSegmentCache": AiorbustSegmentCache}
NODE_DISPLAY_NAME_MAPPINGS = {"AiorbustSegmentCache": "Aiorbust Segment Cache"}
