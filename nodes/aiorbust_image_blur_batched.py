# -*- coding: utf-8 -*-
"""
ComfyUI node - Aiorbust Image Blur (batched).
...
def _stock_callable(node_key="ImageBlur"):
    ...
    if cls is None:
        raise RuntimeError(
            f"[Aiorbust batched] ComfyUI's {node_key} node was not found in "
    ...
    raise RuntimeError(
        f"[Aiorbust batched] Found {node_key} ({cls.__name__}) but none of "
...
def _first_image(result):
    ...
    for attempt in (lambda r: r[0],
        ...
        try:
            ...
            continue
    raise RuntimeError(
        f"[Aiorbust Image Blur] Could not read an IMAGE out of ImageBlur's "
        f"return value ({type(result).__name__})."
    )


class AiorbustImageBlurBatched:
    ...
    def blur(self, image, blur_radius, sigma, frames_per_slice=0):
        ...
        if slice_size >= total:
            print(f"[Aiorbust Image Blur] {total} frame(s) at {width}x{height} — "
        ...
        print(f"[Aiorbust Image Blur] {total} frame(s) at {width}x{height}x{channels} — "
        ...
        logging.info("Aiorbust Image Blur: %d frames blurred in %d slices", total, n_slices)
        return (result,)
...
class AiorbustImageCompositeMaskedBatched:
    ...
    def composite(self, destination, source, x, y, resize_source,
        ...
        n_slices = (total + slice_size - 1) // slice_size
        print(f"[Aiorbust Composite] {total} frame(s) at {width}x{height}x{channels} — "
              f"{n_slices} slice(s) of up to {slice_size} "
              f"({'auto' if frames_per_slice <= 0 else 'manual'}); "
        ...
        logging.info("Aiorbust Composite: %d frames composited in %d slices", total, n_slices)
        return (result,)
