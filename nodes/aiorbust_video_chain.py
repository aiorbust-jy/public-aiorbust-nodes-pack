# -*- coding: utf-8 -*-
"""
ComfyUI nodes - Aiorbust Video Chain (Prepare / Commit).
...
class AiorbustVideoChainSegment:
    ...
    def segment_for(self, segment, overlap, segment_index, video_fps,
        ...
        if source_total > 0:
            ...
            if start >= source_total:
                ...
                raise ValueError(
                    msg + "\n-> Bypass this branch (Aiorbust Group Toggle), or use a longer "
                          "reference video."
                )
...
class AiorbustVideoChainPrepare:
...
class AiorbustVideoChainCommit:
...
try:
    ...
    @PromptServer.instance.routes.post("/aiorbust/video_chain/reset")
    ...
    logging.warning("Aiorbust Video Chain: could not register the reset route (server not available)")
